...
bullet("**On the only fair footing the probe loses to prompting.** On an unseen source the honest probe (0.71) sits below every frontier model prompted zero-shot on the same rows (0.72 to 0.82) and far below the humans (0.91).")
...
para("One family, prompt_injection, scores below chance. So do all nine frontier models. I read all 19 deploy-labelled transcripts and 15 eval-labelled ones myself: the \"deploy\" side is people typing \"I have been PWNED\" in homoglyphs and Cyrillic for a competition scoreboard. A system-prompt transplant rules out the probe-side explanation. The label is contested by its own annotators.")
...
h2("Experiment 1: Reliability at a 1% base rate")
para("Nguyen et al. §5.5 proposes \"integrating these findings into control protocols as a live monitoring system.\" Their evidence is balanced AUROC. A monitor does not see balanced data. "
     "Precision at base rate b is tpr·b / (tpr·b + fpr·(1−b)). I computed it by hand and it matches the pipeline to five decimals. This is the base-rate fallacy applied to a screening test; Saito & Rehmsmeier ([2015](https://doi.org/10.1371/journal.pone.0118432)) make the general case that ROC hides exactly this.")
h3("Results")
...
para("I scanned the 60 source categories for label mixture. The set of positive rates is {0.0, 1.0}. Nothing in between.")
...
para("So under random CV, a classifier that memorises \"this text smells like hackaprompt\" scores perfectly on the label. Is that what happens? Word counts say yes.")
...
para("Source identity is not the same thing as memorisation. Manek ran ROUGE-L completion and perplexity contamination checks on Qwen (methods from Golchin & Surdeanu, [2308.08493](https://arxiv.org/abs/2308.08493), and Ravaut et al., [2404.00699](https://arxiv.org/abs/2404.00699)) and found no support for a narrow memorisation account. What leaks here is cheaper than memorisation: scaffold text, system prompts, the house style of a corpus.")
...
para("56 rows, all injection attacks. mosscap and gandalf rows are labelled eval, hackaprompt rows deploy. Held-out AUROC 0.30 on 4B. The deploy-labelled rows get *higher* P(eval) than the eval-labelled ones.")
img("pub_f_per_family.png", "Fig 8. Left: held-out P(eval) by source family, with prompt_injection split by label. The hackaprompt side scores 0.83, above most eval-labelled families. Right: within-family AUROC exists for only three families; prompt_injection is the inverted one.")
para("Three candidate mechanisms, and what killed each:")
bullet("**C1, the competition string.** \"I have been PWNED\" is in 58% of hackaprompt rows and 0% of the rest of the corpus. Delete every row containing it: AUROC 0.29. Dead.")
...
para("C2 predicted that the unseen GOURD prompt pushes rows toward the middle. It does the opposite. Give it to hackaprompt and hackaprompt's P(eval) rises from 0.84 to 0.94. Swap both prompts and the inversion deepens on both models. The probe does respond to the system prompt, in the direction that makes things worse. What it scores as eval-like is the hackaprompt user turns. C2 is dead.")
...
para("Nguyen et al. ([2507.01786](https://arxiv.org/abs/2507.01786)) built the foundational probe on Llama-3.3-70B and proposed monitoring. Their probe is a normalised mean-difference direction, the contrastive-activation-addition construction of Panickssery et al. ([2312.06681](https://arxiv.org/abs/2312.06681)); mine is a trained logistic regression, which they list as future work. They trained and tested on synthetic contrastive prompts and SAD, never on real transcripts. And they wrote this in §4.2.2: \"test-classified prompts often contain more structured or formatted elements.\" They observed the confound and did not test it.")
...
bullet2("The benchmark: 976 real transcripts, 61 sources, two annotators, nine frontier models' judgments released. Dataset [jjpn2/eval_awareness](https://huggingface.co/datasets/jjpn2/eval_awareness), code [jjpn97/eval_awareness](https://github.com/jjpn97/eval_awareness).")
...
bullet("[Linear Control of Test Awareness](https://arxiv.org/abs/2505.14617), Abdelnabi & Salem, and [Steering Evaluation-Aware Language Models to Act Like They Are Deployed](https://arxiv.org/abs/2510.20487), Hua, Qin, Marks & Nanda")
...
suspicion(3, "maybe the honest number is noise",
          ...
          "Rejected for the corpus-wide 0.71. Not rejected for smolagents' 0.63, which lives inside the envelope.",
          "~30min.")
suspicion(4, "the bigger model will rescue it",
          "Prior work reports scaling laws for eval-awareness.",
          "Same pipeline on Qwen3-8B.",
          "Two OOMs first: shared L40S cards, 33 GiB already resident on the first one. A free-memory preflight now fails fast. Third submission: leaky 0.960, honest 0.668. Intervals overlap at the edges.",
...
suspicion(5, "prompt_injection at 0.30 is a sign bug, or the PWNED string",
          ...
          "Nine of nine frontier models invert, GPT-4o at 0.10. Annotators 0.91 vs 0.35, r = −0.24 in-family. PWNED-removed: 0.291.",
...
suspicion(6, "it's the GOURD system prompt",
          ...
          "Reproduces to 0.004. Then: hackaprompt with GOURD goes *up*, 0.84 to 0.94. Both swapped: 0.306 to 0.259 on 4B, 0.512 to 0.299 on 8B.",
          "Rejected. The inversion is in the user turns. C3 stands, on my reading of 34 transcripts.",
          "~2h including one GPU job (job 7704957, 3 minutes of compute).")
...
suspicion(8, "the agent's numbers are wrong",
          ...
          "All six reproduce. 32 tests pass, but only after sourcing the project env; the default conda env has no sklearn and collection fails outright. Two corrections against my own interest: the honest floor across all 28 configs is 0.465, and my \"18 independent judges\" counted the two leaky probes and a derived human mean.",
          "Confirmed, with the two corrections folded into the text above.",
          "~2h. Plus one stalled job: threaded BLAS oversubscribing 47 small refits, killed (rc=143 in sens.log), resubmitted with OMP_NUM_THREADS=1, finished in two minutes. Lesson: pin your thread counts before you pin your hopes.")
img("pub_m_verification.png", "Fig 12. The verification terminal: test suite, the claim-3 recompute against the locked numbers, the by-hand Bayes check, and the transplant reproduction check.")
